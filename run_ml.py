...
def analyze_csv(input_path, output_dir=None):
    ...
    if "Time_Second" not in df.columns:
        raise ValueError(f"{file_name} does not contain a Time_Second column.")
...
def analyze_all_csvs_in_folder():
    ...
    for file_name in csv_files:
        ...
        try:
            result = analyze_csv(input_path, current_folder)
            print(f"✅ Success! Analyzed {result['rows']} seconds. Found {result['anomalies']} anomalies.")
            print(f"💾 Saved Excel file as -> {result['output_name']}\n")
        except Exception as e:
            print(f"❌ Error processing {file_name}: {e}\n")


def main():
    parser = argparse.ArgumentParser(description="Throughput ML analyzer")
    parser.add_argument("--input", help="Analyze a single CSV file")
    parser.add_argument("--output-dir", help="Directory for the generated Excel report")
    args = parser.parse_args()

    if args.input:
        result = analyze_csv(args.input, args.output_dir)
